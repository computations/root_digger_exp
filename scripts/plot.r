...
timing_data[taxa=="10" & sites == "1000" & program=="iq", `:=`(time_scaled, time )]#* (1 - .37))]
timing_data[taxa=="50" & sites == "1000" & program=="iq", `:=`(time_scaled, time )]#* (1 - .59))]
timing_data[taxa=="10" & sites == "8000" & program=="iq", `:=`(time_scaled, time )]#* (1 - 0.58))]
timing_data[taxa=="50" & sites == "8000" & program=="iq", `:=`(time_scaled, time )]#* (1 - 0.59))]
timing_data[program=="rd_es", `:=`(time_scaled, time)]
timing_data[program=="rd_nes", `:=`(time_scaled, time)]
...
time_boxplot = ggplot(data = timing_data, aes(y = time_scaled, x = program, fill = program)) +
